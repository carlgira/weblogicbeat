...
func (bt *Weblogicbeat) ServerStatusEvent() {
	...
	for _, server_name := range bt.config.ServerNames {
		resp_server_status, err_server_status := resty.R().
			...
			Get(bt.config.Host + "/management/weblogic/latest/domainRuntime/serverRuntimes/" + server_name + "?links=none&fields=name,state,healthState")

		if err_server_status != nil {
			bt.SendErrorEvent(server_name, "server_status", fmt.Sprintf("%v", err_server_status))
			continue
		}
		...
		resp_server_jvm, err_server_jvm := resty.R().
			...
			SetHeader("X-Requested-By", "weblogicbeat").
			SetBasicAuth(bt.config.Username, bt.config.Password).
			Get(bt.config.Host + "/management/weblogic/latest/domainRuntime/serverRuntimes/" + server_name + "/JVMRuntime?links=none&fields=heapSizeCurrent,heapFreeCurrent,heapFreePercent,heapSizeMax")

		if err_server_jvm != nil {
			bt.SendErrorEvent(server_name, "server_status", fmt.Sprintf("%v", err_server_jvm))
			continue
		}
	...
	for _, server_name := range bt.config.ServerNames {
		for _, datasource := range bt.config.Datasources {
			resp_ds, error_ds := resty.R().
				...
				SetHeader("X-Requested-By", "weblogicbeat").
				SetBasicAuth(bt.config.Username, bt.config.Password).
				Get(bt.config.Host + "/management/weblogic/latest/domainRuntime/serverRuntimes/" + server_name + "/JDBCServiceRuntime/JDBCDataSourceRuntimeMBeans/" + datasource + "?links=none&fields=activeConnectionsCurrentCount,activeConnectionsAverageCount,connectionsTotalCount,enabled,state,name")

			if error_ds != nil {
				bt.SendErrorEvent(server_name, "datasource_status", fmt.Sprintf("%v", error_ds))
				continue
			}
			...
			_, error_ds_test := resty.R().
				...
				SetHeader("X-Requested-By", "weblogicbeat").
				SetBasicAuth(bt.config.Username, bt.config.Password).
				Get(bt.config.Host + "/management/weblogic/latest/domainRuntime/serverRuntimes/" + server_name + "/JDBCServiceRuntime/JDBCDataSourceRuntimeMBeans/" + datasource + "/testPool")
	...
	for _, server_name := range bt.config.ServerNames {
		for _, application := range bt.config.Applications {
			resp_app, err_app := resty.R().
				...
				SetHeader("X-Requested-By", "weblogicbeat").
				SetBasicAuth(bt.config.Username, bt.config.Password).
				Get(bt.config.Host + "/management/weblogic/latest/domainRuntime/serverRuntimes/" + server_name + "/applicationRuntimes/" + application + "?links=none&fields=name,healthState")

			if err_app != nil {
				bt.SendErrorEvent(server_name, "application_status", fmt.Sprintf("%v", err_app))
				continue
			}
			...
			resp_app_comp, err_app_comp := resty.R().
				...
				SetHeader("X-Requested-By", "weblogicbeat").
				SetBasicAuth(bt.config.Username, bt.config.Password).
				Get(bt.config.Host + "/management/weblogic/latest/domainRuntime/serverRuntimes/" + server_name + "/applicationRuntimes/" + application + "/componentRuntimes?fields=openSessionsCurrentCount,sessionsOpenedTotalCount,openSessionsHighCount,applicationIdentifier,status,componentName&links=none")

			if err_app_comp != nil {
				bt.SendErrorEvent(server_name, "application_status", fmt.Sprintf("%v", err_app_comp))
				continue
			}
...
func (bt *Weblogicbeat) ThreadStatusEvent() {
	...
	for _, server_name := range bt.config.ServerNames {
		resp_thread_status, err_thread_status := resty.R().
			...
			SetHeader("X-Requested-By", "weblogicbeat").
			SetBasicAuth(bt.config.Username, bt.config.Password).
			Get(bt.config.Host + "/management/weblogic/latest/domainRuntime/serverRuntimes/" + server_name + "/threadPoolRuntime?links=none&fields=overloadRejectedRequestsCount,pendingUserRequestCount,executeThreadTotalCount,healthState,stuckThreadCount,throughput,hoggingThreadCount")

		if err_thread_status != nil {
			bt.SendErrorEvent(server_name, "thread_status", fmt.Sprintf("%v", err_thread_status))
			continue
		}
